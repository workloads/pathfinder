...
char* WifiClient::generateHostname() {
    ...
    Log::debug(logTag, "Generated hostname: `%s`", wifiHostname);
...
bool WifiClient::disconnect() {
    ...
    Log::info(logTag, "Disconnecting from SSID `%s`", wifiSsid);
...
bool WifiClient::connect() {
    ...
    Log::debug(logTag, "Generating Wi-Fi hostname");
    char* wifiHostname = generateHostname();

    // Set Wi-Fi hostname
    Log::debug(logTag, "Setting Wi-Fi hostname to `%s`", wifiHostname);
    WiFi.setHostname(wifiHostname);

    // Set Wi-Fi auto-reconnect option
    const char* wifiAutoReconnectStr = WifiClient::autoReconnect ? "true" : "false";
    Log::debug(logTag, "Setting Wi-Fi auto-reconnect option to `%s`", wifiAutoReconnectStr);
    ...
    Log::info(logTag, "Attempting to connect to SSID `%s`", wifiSsid);
    ...
    while (attemptsLeft > 0) {
        ...
        switch (WiFi.status()) {
            case WL_NO_SHIELD:
                Log::error(logTag, "Device unable to find Radio");
                return false;

            case WL_NO_SSID_AVAIL:
                Log::info(logTag, "Device unable to find SSID `%s` after %d attempts", wifiSsid, attemptCount);
                break;

            case WL_CONNECT_FAILED:
                Log::warning(logTag, "Device unable to connect");
                break;

            case WL_CONNECTION_LOST:
                Log::info(logTag, "Device lost connection");
                break;

            case WL_SCAN_COMPLETED:
                Log::debug(logTag, "Device completed scan");
                break;

            case WL_DISCONNECTED:
                Log::info(logTag, "Device disconnected");
                break;

            case WL_CONNECTED:
                Log::info(logTag, "Device connected with IP `%s`", WiFi.localIP().toString().c_str());
            ...
            default:
                Log::warning(logTag, "Unhandled Wi-Fi status `%d` on attempt %d", WiFi.status(), attemptCount);
                break;
        }
        ...
        if (attemptsLeft <= 0) {
            Log::error(logTag, "Failed to connect to SSID `%s` after %d attempts", wifiSsid, reconnectAttempts);
            disconnect();
...
void WifiClient::reconnect() {
    ...
    Log::info(logTag, "Disconnecting from Wi-Fi");
    disconnect();

    Log::info(logTag, "Reconnecting to Wi-Fi SSID `%s`", wifiSsid);
    WiFi.reconnect();
    ...
    Log::info(logTag, "Connected with IP `%s`", ipAddress);
}
...
void WifiClient::scanNetworks() {
    ...
    // Start Wi-Fi scan
    Log::info(logTag, "Starting Wi-Fi scan");
    int networkCount = WiFi.scanNetworks();

    // Check if Wi-Fi scan was successful
    if (networkCount == -1) {
        Log::error(logTag, "Failed to complete Wi-Fi scan");
    } else {
        Log::info(logTag, "Wi-Fi scan completed, found %d networks", networkCount);

        // Log details of found networks
        for (int i = 0; i < networkCount; i++) {
            Log::info(logTag, "%d: `%s` (RSSI: %d, Encryption: %s)", i + 1, WiFi.SSID(i).c_str(), WiFi.RSSI(i), (WiFi.encryptionType(i) == WIFI_AUTH_OPEN) ? "🔓" : "🔒");
        }
    }
...
void WifiClient::startMdnsService(const char* hostname, const char* instance_name) {
    ...
    Log::debug(logTag, "Initializing mDNS service");
    esp_err_t err = mdns_init();

    if (err) {
        Log::error(logTag, "Failed to initialize mDNS: %s", esp_err_to_name(err));
        return;
    }

    Log::debug(logTag, "Setting mDNS hostname to `%s`", hostname);
    mdns_hostname_set(hostname);

    Log::debug(logTag, "Setting mDNS instance name to `%s`", instance_name);
