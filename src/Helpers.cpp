...
const char *helperIpAddressToString(IPAddress inputAddr) {
    ...
    Logger::info(__func__, "IP Address: %s", output);
    return output;
}

int helperStringToLogLevel(const char *levelString) {
    const LogLevelMap *logLevels     = Logger::getLogLevels();
    int                logLevelsSize = Logger::getLogLevelsSize();

    for (int i = 0; i < logLevelsSize; i++) {
        if (strcmp(logLevels[i].name, levelString) == 0) {
            Logger::info(__func__, "Result: %d", logLevels[i].level);
...
uint8_t helperPercentageToInteger(int percentage) {
    ...
    if (percentage < 0) {
        Logger::warning(logTag, "Percentage outside lower bound, setting to 0");
        percentage = 0;
    } else if (percentage > 100) {
        Logger::warning(logTag, "Percentage outside upper bound, setting to 100");
        percentage = 100;
    }
