...
void espNowSingleDevSend(String inputMac, byte devCodeIn, float bIn, float sIn, float eIn, float hIn, byte cmdIn, String messageIn) {
    ...
    esp_err_t result = esp_now_send(macArray, (uint8_t *)&espNowMessage, sizeof(struct_message));
...
void espNowSingleDevFlowCtrl() {
    ...
    esp_err_t result = esp_now_send(singleFollowerDev, (uint8_t *)&espNowMessage, sizeof(struct_message));
